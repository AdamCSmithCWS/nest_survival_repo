...
mod_prep <- cmdstanr::cmdstan_model(stan_file = "stan_model_like_tobias_roth.stan")

run_stan_model_like_tobias<- mod_prep$sample(data=stan_data,
                                             refresh = 100,
                                             iter_sampling = 100, #just for initial attempts, but use the defaults when running for real
                                             iter_warmup = 500,
                                             parallel_chains = 4)#just for initial attempts, but use the defaults when running for real
